...
//        doUnaryCallWithDeadline(channel);
    ...
    private void doBiDiStreamingCall(ManagedChannel channel) {
        ...
        Arrays.asList("John", "Joe", "Marc", "Patricia").forEach(
                name -> {
    ...
    private void doUnaryCallWithDeadline(ManagedChannel channel) {
        ...
        GreetWithDeadlineRequest request = GreetWithDeadlineRequest
                .newBuilder()
                .setGreeting(Greeting.newBuilder().setFirstName("John"))
                .build();
        ...
        try {
            System.out.println("Sending a request with a deadline of 1000 ms");
            GreetWithDeadlineResponse response = blockingStub.withDeadlineAfter(1000, TimeUnit.MILLISECONDS).greetWithDeadline(request);
        ...
        try {
            ...
            GreetWithDeadlineResponse response = blockingStub.withDeadline(Deadline.after(100, TimeUnit.MILLISECONDS)).greetWithDeadline(request);
            System.out.println(response.getResult());
        } catch (StatusRuntimeException e) {
            if (e.getStatus().getCode() == Status.Code.DEADLINE_EXCEEDED) {
                System.out.println("Deadline has been exceeded, we don't want the response");
